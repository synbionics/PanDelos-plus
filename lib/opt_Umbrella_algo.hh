...
std::vector<std::vector<node_id_t>> split_until_max_k(
                ...
                const Graph& original_network, const std::unordered_map<int, std::string>& seq_genome,
                bool is_weighted = false)
{
    
    SubGraph component_subnet(original_network, component);
    ...
    while (!to_process.empty()) {
        ...
        if (get_max_collision(community, component_subnet, seq_genome) > 0) {
            std::vector<std::vector<node_id_t>> subresult = split_until_max_k(community, original_network, seq_genome);
