...
std::unordered_map<std::pair<node_id_t,node_id_t>, weight_t, PairHash> calculate_edge_betweenness(
    ...
    if(gview.get_number_of_nodes() >= THRESHOLD){
        ...
        for(node_id_t s : gview.get_nodes()){
            
            auto task = [s, &gview, is_weighted, &edge_contributions, &contributions_mutex]() {
                ...
                for (node_id_t v : gview.get_nodes()) {
...
int get_max_collision(std::vector<node_id_t> component, const GraphView& network,
    const std::unordered_map<node_id_t, std::string>& seq_genome){
...
std::vector<std::vector<node_id_t>> girvan_newman(GraphView& network, UmbrThreadPool& pool, const node_id_t THRESHOLD, bool is_weighted){
                ...
                bool is_weighted = false)
{
    ...
    std::vector<std::vector<node_id_t>> tmp_communities = girvan_newman(component_subnet_view,pool,THRESHOLD,is_weighted);
    ...
    while (!to_process.empty()) {
        ...
        if (get_max_collision(community, component_subnet_view, seq_genome) > 0) {
            std::vector<std::vector<node_id_t>> subresult = split_until_max_k(community, network, seq_genome, pool, THRESHOLD, is_weighted);
