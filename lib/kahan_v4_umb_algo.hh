...
std::unordered_map<std::pair<node_id_t,node_id_t>, weight_t, PairHash> calculate_edge_betweenness(
    ...
    if(g.get_number_of_nodes() > COMPONENT_SIZE_THRESHOLD){
        ...
        for(node_id_t s : g.get_nodes()){
            ...
            auto task = [s, &g, is_weighted, &edge_contributions, &contributions_mutex]() {
                
                std::unordered_map<std::pair<node_id_t, node_id_t>, weight_t, PairHash> local_edge_betweenness;
                ...
                while (!stack.empty()) {
                    ...
                    for (node_id_t v : pred[w]) {
                        ...
                        local_edge_betweenness[edge] += coefficent; 
                        info[v].delta += coefficent;
                    }
                }
                
                {
                    std::lock_guard<std::mutex> lock(contributions_mutex);
                    for (const auto& [edge, val] : local_edge_betweenness) {
                        edge_contributions[edge].push_back(val);
                    }
                }
            };
            
            bw_pool.execute(task);
        }
        
        bw_pool.wait();
        
        for (auto& [edge, contributions] : edge_contributions) {
            //std::sort(contributions.begin(), contributions.end());
            
            double sum = 0.0;
            double compensation = 0.0;
            
            for (double val : contributions) {
...
int get_max_collision_fast(const std::vector<node_id_t>& component, 
                          const Graph& network,
                          const std::unordered_map<node_id_t, std::string>& seq_genome) {
    
    // Raggruppiamo i nodi per genoma
    std::unordered_map<std::string, std::vector<node_id_t>> genome_groups;
    for (node_id_t n : component) genome_groups[seq_genome.at(n)].push_back(n);

    int global_max_k = 0;

    for (auto& [genome, nodes] : genome_groups) {
        if (nodes.size() <= 1) continue;

        // Per ogni genoma, creiamo un set rapido dei nodi presenti
        std::unordered_set<node_id_t> nodes_in_group(nodes.begin(), nodes.end());

        for (node_id_t u : nodes) {
            int same_genome_neighbors = 0;
            // Invece di guardare tutti i nodi, guardiamo solo i vicini reali di u
            for (const auto& [v, _] : network.get_neighbors(u)) {
                if (nodes_in_group.count(v)) {
                    same_genome_neighbors++;
                }
            }
            // S_k = (Totale nodi con quel genoma - 1) - (vicini reali con quel genoma)
            int s_k = (nodes.size() - 1) - same_genome_neighbors;
            if (s_k > global_max_k) global_max_k = s_k;
        }
    }
    return global_max_k;
}
