...
for line in open(iseqs, 'r'):
    if i%2 == 0:
        cols = line.strip().split(sequenceSeparator)
...
for line in open(inet, 'r'):
    cols = line.strip().split(netSeparator)
    if float(cols[2]) != 0.0:
        cols[0] = int(cols[0])
        cols[1] = int(cols[1])
        cols[2] = float(cols[2])
...
pos = nx.spring_layout(pnet)
nx.draw(pnet, pos, with_labels=True, node_size=150, node_color='skyblue', font_size=8, edge_color='gray', width=0.5)

labels = nx.get_edge_attributes(pnet, 'weight')
nx.draw_networkx_edge_labels(pnet, pos, edge_labels=labels)
