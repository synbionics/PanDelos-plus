...
net_clug_path="${scripts_path}parallel_net"
...
echo "Compiling cpp parallel for clustering (O3, can be done in advance)"
g++ -O3 $scripts_path/umb_netclu.cc ./lib/umb_algo.hh ./lib/UmbrThreadPool.hh ./lib/Graph.hh ./lib/SubGraph.hh ./lib/GraphView.hh ./lib/types.hh -o $scripts_path/parallel_net
...
echo "" >> $tmp;
"$net_clug_path" "$inFile" "$outFile.net" >> $tmp

if [ $? -ne 0 ]; then
    echo "Error running umb_netclu.cc"
    cat $tmp
    exit 1
fi
